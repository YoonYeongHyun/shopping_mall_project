...
int cnt = 0;
if(Integer.parseInt(category) < 100){
	cnt = productDAO.getProductCount(category, search);
}else{
	cnt = productDAO.getProductSubCount(category, search);
}
...
if(Integer.parseInt(category) < 100){
	productList = productDAO.getProductsCategoryList(category, search, startRow, pageSize);
}else{
	productList = productDAO.getProductsSubCategoryList(category, search, startRow, pageSize);
}
%>
<script>
		...
		if(cnt > 0){
			...
			for (int i=startPage; i<=endPage; i++){
				if(currentPage == i){
					...
					%><a href='shoppingAll.jsp?pageNum=<%=i%>&category=<%=category%>&search=<%=search%>&code=2'>
						<div id='p_box'> <%=i %> </div>
					</a><% 
				}
			}
